# Week 14 Tasking
**Story**: As a CSE 397 Student, I want to complete the following tasks this week, so that I can return and report on my success as a valuable member of the team. 

See [Weekly Tasking](https://github.com/byui-cse397/2026.2SprCSE397PCP/blob/development/project/management/scheduling/Week14.md) for each weeks tasking.

## Agile Issue (05 pts)
Update the GitHub issue attributes
* **Author**: Joseph Tolley  
* **Labels**: Documentation 
* **Type**:
* **Milestone**: Week 14, Version 6.0 
* **Projects**: CSE397PCP
* **Development (Branch)**: Tolley_Wk14

## Training/Class Meeting Minutes(05 pts)
 - [x] Training/Class Meeting - No meeting this week

## Individual Work (10 pts - 5 for [Gold](../Tolley_CertificationReport.md), 5 for [Training](../Tolley_dossier.md)
 - [x] Professional Networking Certification: [Career Services Gold Certificate (every week)](../Tolley_CertificationReport.md)
 - [X] Professional Training- All Completed
 - [X] [Dossier Completed](../Tolley_dossier.md)
## Project Teamwork Meeting [Minutes]()(10 pts)
 - [x] [Weekly Meeting with team]()

## Project Individual Work (10 pts)
In Agile Daily Scrum meetings each individual are asked three questions:
* What did I do yesterday to help meet the Sprint Goal?
  - I helped with updating the presentation and prepared my slides of responsibility. I worked on a finalized APK that should be able to work on any android device.
* What will I do today to help meet the Sprint Goal?
  - I will present with my team, our RxNow App. I will contribute to the final version and clean up our repo's.
* Do I see any impediment that prevents me or the team from meeting the Sprint Goal.
  - If everyone shows up and takes care of their area of influence, it will go well. If not, then we will adjust and if needed we are capable of presenting or taking on each other's tasks.
### What did you do yesterday to help meet the Sprint Goal?
* Back-End Lead (current week)
 - [x] Issue Tasking (every week)[see product repo's issues](https://miro.com/app/board/uXjVHW1B9x4=/) (50 pts - 5 per week @ 10 pts each)
* [Feature](https://miro.com/app/board/uXjVHW1B9x4=/) Tasking completed This week (50 pts - 10 pts per completed task)
   - [x] 14.1. [Version-6.0](https://github.com/louisramos23/RXNOW/tree/main/Documents/VDD%20documents)
   - [x] 14.2. [Final APK](https://github.com/louisramos23/RXNOW/tree/main/Frontend/build/app/outputs/flutter-apk)
   - [x] 14.3. [Contribute to Powerpoint](https://docs.google.com/presentation/d/11r-jkIlPSOXg6ONkI74jsS2wVa-TJwot_6zLJskakU4/edit?slide=id.g3f552822d11_6_1#slide=id.g3f552822d11_6_1)
   - [x] 14.4  [Presentation]()
   - [x] 14.5  [Update documentation](https://github.com/louisramos23/RXNOW/tree/main/Documents)
   - [x] 14.6  Clean up Repo's (class, project, team)
      
* LAST WEEK, no future tasking! Project in its [final form](https://github.com/louisramos23/RXNOW/tree/main) before handoff to next team

### Rubric
*	05 pts – Agile Information
*	05 pts – Class/Training Meeting Notes
*	10 pts – Individual Work (CC-Gold, Training)
*	10 pts – Team planning meeting
*	10 pts – Planning Tasking (Feature or Role)
*	05 pts – Research Tasking
*	50 pts	- Issue Tasking (8 listed)
*	05 pts – Next Weeks Planning (5 listed)
